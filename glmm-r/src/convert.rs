//! Pure translation helpers — no `extendr` types anywhere in this file, so
//! these run under plain `cargo test` with no R interpreter involved.
//!
//! Mirrors `glmm-python/src/convert.rs` — change together: the two ports
//! share one family/link vocabulary.

use glmm::{BinomialLink, Family, GammaLink, NegBinomialLink, PoissonLink};

/// Maps the ports' `family`/`link` strings to `glmm::Family`. `family` and
/// `link` are already validated against the full (0.1.1-aware) spec table by
/// `r/R/fastglmm.R` before this ever runs — the "not yet implemented in
...
/// `fastglmm()` and exists only for direct callers of this crate.
