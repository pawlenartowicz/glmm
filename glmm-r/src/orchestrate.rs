...
//! abort across the FFI. No `extendr` types here — plain-Rust, unit-testable.
//!
//! Mirrors `glmm-python/src/orchestrate.rs` — change together. The two ports
//! bind the kernel at the same level, so a fix or policy change on one
//! side (e.g. the nagq warn-and-strip eligibility) must land on both.
...
pub struct FitResult {
    ...
    /// field's not-comparable-across-models caveat; see `glmm::Fit::deviance`.
    pub deviance: f64,
...
pub fn run_fit(
    ...
    // (mirrored so the two ports agree): nothing inapplicable may reach the
    // kernel, whose shape check is an `assert!`, and a Rust panic across the FFI
    // is not an acceptable user error. The shape is only knowable here, after
    // `lower()`, so this is the
    // strip site; `fastglmm()` emits the message as an R `warning()`.
    // Eligibility mirrors
