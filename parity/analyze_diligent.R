...
#
#   Rscript analyze_diligent.R [glmm.jsonl] [oracle.jsonl]
#   (defaults: results/diligent/glmm.jsonl, results/diligent/lme4.jsonl)
#
...
# hard gate -- beta/stddev/se/corr are the parity gates.
suppressMessages({ library(jsonlite) })

parity_dir <- normalizePath(dirname(sub(
  "--file=", "", grep("--file=", commandArgs(FALSE), value = TRUE))))
source(file.path(parity_dir, "tol.R"))
args <- commandArgs(TRUE)
glmm_path   <- if (length(args) >= 1) args[1] else
  file.path(parity_dir, "results", "diligent", "glmm.jsonl")
oracle_path <- if (length(args) >= 2) args[2] else
  file.path(parity_dir, "results", "diligent", "lme4.jsonl")
out_dir <- file.path(parity_dir, "reports", "diligent")
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

glmm <- read_jsonl(glmm_path); oracle <- read_jsonl(oracle_path)
manifest <- fromJSON(file.path(parity_dir, "manifest_diligent.json"),
...
# measurably worse optimum) stays a named parity failure, not hand-waved.
...
# Parity failures = mismatches + real glmm/both engine failures. Everything else
...
#     via adjudicate_diligent.R (change together): glmer-Laplace on the same
...
writeLines(c(
  ...
  sprintf("# %d parity failures of 510 cells.", length(fail_ids)),
...
cat(sprintf(paste0("  1. Laplace vs lme4      : %d/%d cells match (%d parity-fail, ALL same-optimum: ",
            ...
            n_otime_lap))
cat(sprintf("  2. int1 scalar AGQ k=7  : %d/%d joinable match lme4 (%d parity-fail)\n",
            c2$ok, c2$n, c2$fail))
...
# 4 structures per table in n_theta order). Each cell shows the worst parity gap
...
html <- c(
  ...
  sprintf(paste0('<p class="note">510-cell accuracy sweep (full-AGQ vector-RE spec, Part 6). ',
    'glmm result rows: %d/510; oracle rows: %d/510. Each cell = worst gated parity gap ',
  ...
  sprintf('<div class="claim"><b>2. int1 scalar AGQ (15 cells, %d joinable):</b> %d/%d &equiv; lme4 glmer(nAGQ=7) across the size&times;balance&times;regime sweep &mdash; %d parity-fail. Includes the 6 aggregated-binomial <code>bina_int1_*</code> cells, now fit via AGQ&times;prior-weights (gate lifted 2026-07-14).</div>',
