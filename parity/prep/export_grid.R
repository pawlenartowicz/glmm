...
# writes manifest_grid.json (committed — THE reproducibility artifact; CSVs are
# derived) and data_simulated/grid/<case_id>.csv (gitignored). Deliberately
...
grid_dir <- file.path(parity_dir, "data_simulated", "grid")
...
manifest <- list(schema = "glmm-grid-manifest/1",
                 generated_by = "prep/export_grid.R",
...
write(toJSON(manifest, auto_unbox = TRUE, pretty = TRUE, digits = NA, null = "null"),
      file.path(parity_dir, "manifest_grid.json"))
...
write(toJSON(diligent, auto_unbox = TRUE, pretty = TRUE, digits = NA, null = "null"),
      file.path(parity_dir, "manifest_diligent.json"))
