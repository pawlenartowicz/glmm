...
#   Rscript analyze_grid.R <glmm.jsonl> <mixedmodels.jsonl> [lme4.jsonl]
...
if (length(args) < 2) stop("usage: analyze_grid.R glmm.jsonl mm.jsonl [lme4.jsonl]")
...
out_dir <- file.path(parity_dir, "reports", "grid")
...
# JIT guard: grid_fit.jl warm-up-fits each cell and records compile_seconds
...
manifest <- fromJSON(file.path(parity_dir, "manifest_grid.json"),
...
# than tier-0's 1e-3 because this is a false-stop guard, not a parity gate).
...
cat("\nNOTE: wall/per-eval columns are meaningful ONLY for clock-locked passes",
    "(check results/grid/run_meta_*.json no_turbo==1).\n")
