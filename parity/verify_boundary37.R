...
# machinery and lmm tolerance bands as analyze_diligent.R (docs/GLMM/plans/
...
# analyze_diligent.R applies to GLMMadaptive rows).
#
...
mm_path   <- if (length(args) >= 2) args[2] else
  file.path(parity_dir, "results", "diligent", "mixedmodels.jsonl")
...
# same offset analyze_diligent.R's glmm_dev_aligned() uses for lme4. A tiny
...
# read_jsonl/stddevs_of/corrs_of live in tol.R (shared with analyze_diligent.R).
...
for (cid in target_ids) {
  ...
  # analyze_diligent.R; change together).
  ...
  dg <- NA_real_
  # Verdict strings below are matched literally by analyze_diligent.R's
