//! Shared helpers between the two glmm parity harnesses: `fit.rs` (the frozen
//! per-dataset corpus, `parity_fit` example) and `grid_fit.rs` (the
...
/// The dataset-name+source variant (`fit.rs`'s `read_csv`) delegates here —
...
/// incidence/weights_col` (mirrors fit.jl:44-46) so jl_formula's `prop ~ ...`
