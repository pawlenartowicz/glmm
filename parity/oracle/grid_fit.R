...
# share this file (env-var-driven, mirrors grid_fit.rs's GRID_MANIFEST/GRID_OUT):
#
# - manifest_grid.json (default): lazy anchor -- fits ONLY the cells named in
#   GRID_TODO (mismatch cells + audit sample from analyze_grid.R) or GRID_ONLY.
...
# matrix (diligent run, spec Part 6). Mirrors oracle/fit.R::varcomp_of and
# oracle/fit_m3_goldens.R::varcomp_of (the sd_se-less form -- the theta-Hessian
# SE fit_m3_goldens.R attaches is a per-rung numDeriv Hessian, too expensive to
...
# GLMMadaptive tightened controls (fit_m3_goldens.R precedent): the package's
...
for (cell in cells) {
  ...
  rec <- tryCatch({
    ...
    if (is_vec_agq) {
      ...
      # convention, `harness_common.rs::lower_dataset_generic`'s `prop`
