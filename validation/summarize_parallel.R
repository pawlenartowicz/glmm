...
# `parallel` feature buys over glmm's own serial path. Deliberately not a validation
...
read_engine <- function(dir_name) {
  dir <- file.path(suite_dir, "results", dir_name)
...
# Serial glmm results are split empirical/simulated (validation/README.md) -- merge
...
read_glmm_serial <- function() {
  files <- unlist(lapply(c("empirical", "simulated"), function(s)
    list.files(file.path(suite_dir, "results", paste0("glmm_", s)),
...
if (is.null(serial)) stop("no serial glmm results -- run engines/glmm.rs (feature off, taskset -c 1) first")
...
meta_path <- file.path(suite_dir, "results", "glmm_parallel", "run_meta.json")
...
agq_path <- file.path(suite_dir, "results", "glmm_parallel", "agq_timings.csv")
