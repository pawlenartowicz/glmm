#!/usr/bin/env Rscript
# lme4 reference fits over the validation datasets -> results/lme4_{empirical,simulated}/<dataset>.json.
#
...
fit_one <- function(spec) {
  ...
  if (spec$family == "gaussian") {
    ...
    # Emit BOTH so the validation check compares like method to like: lme4_rx vs
...
# VALIDATION_ONLY=<name>[,<name>...]: fit only the named datasets. This is how a NEW
