...
#
#   ./run.sh glmm|mixedmodels|lme4 <pass-tag> [timeout-seconds]
...
case "$ENGINE" in
  glmm)        CMD=(cargo run --quiet --release --manifest-path "$ROOT/../Cargo.toml" -p validation --example grid_fit)
               # 3x the per-fit budget: fit.rs runs warm-up + 2 timed
               # fits per cell (min-of-2 reported) — change with fit_cell's
               # timing protocol
               TIMEOUT="${TIMEOUT:-30}" ;;
  mixedmodels) CMD=(julia --project="$ROOT" "$HERE/fit.jl")
               # 2x the per-fit budget: fit.jl double-fits each cell (JIT
