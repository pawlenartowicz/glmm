...
# analyze.R). Structures grouped 4 per table in n_theta order: rows =
...
csv <- if (length(args) >= 1) args[1] else
  file.path(suite_dir, "reports", "status_map.csv")
...
# -2logL from analyze.R (lower = better fit); &#8595; = that engine reached
...
html <- c(sprintf(paste0(
  ...
  'Julia 1.12.6 / MixedModels.jl per the pinned validation env.</p>',
