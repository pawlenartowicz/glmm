...
//! inputs [`glmm::fit_cold`] consumes — a row-major design matrix, a structure-only
//! [`glmm::ModelSpec`], per-row [`glmm::GroupIds`], and a defaulted
//! [`glmm::FitOptions`]. Two stages, cut on whether data is needed:
//!
...
//! ```ignore
//! let lo = glmm_formula::lower("y ~ x*z + (1|g)", &table, Family::Gaussian)?;
