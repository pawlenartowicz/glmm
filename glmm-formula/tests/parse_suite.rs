...
use glmm_formula::{parse, ParsedFormula, RandomEffect, Term};
...
fn duplicate_interaction_grouping_var_errors() {
    ...
    assert!(matches!(
        err,
        glmm_formula::ParseError::DuplicateGroupingVar { .. }
    ));
}
...
fn intercept_suppression_rejected() {
    for f in [
        ...
        assert!(
            matches!(
                parse(f),
                Err(glmm_formula::ParseError::RandomInterceptSuppressionUnsupported)
            ),
